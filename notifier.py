...
def send_feishu_hot_products(webhook: str, products: list, category: str = "家居家装"):
    ...
    推送热销商品榜飞书卡片（紫色）
    兼容两种数据格式：
    1. 精选联盟格式: {title, price, monthly_sales, commission_rate, product_url}
    2. 蝉妈妈格式:   {title, commission, daily_sales, total_sales, conversion, rank, category, source}
    """
    if not webhook or "填入你的token" in webhook:
        return
    if not products:
    ...
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    # 检测数据来源
    is_chanmama = bool(products[0].get("source") == "蝉妈妈" or "daily_sales" in products[0])
    ...
    elements = [
        {
            ...
            "text": {
                "tag": "lark_md",
                "content": (
                    f"数据来源：**{'蝉妈妈商品销量榜' if is_chanmama else '精选联盟'}**　　"
                    f"类目：**{category}**　　"
                    f"共 **{len(products)}** 个商品　　{now}"
                )
            }
        },
        {"tag": "hr"},
    ...
    for i, p in enumerate(products, 1):
        ...
        if is_chanmama:
            # 蝉妈妈字段（均为字符串）
            daily  = p.get("daily_sales", "未知")
            total  = p.get("total_sales", "")
            comm   = p.get("commission", "0%")
            conv   = p.get("conversion", "")
            url    = "https://www.chanmama.com/promotionRank/tikGoodsSale/"

            # 日销量标志
            if "25w" in daily or "50w" in daily:
                badge = f"🔥 日销 {daily}"
            elif "10w" in daily:
                badge = f"📈 日销 {daily}"
            else:
            ...
            if conv:
                lines.append(f"转化率 {conv}　　[查看榜单]({url})")
            else:
                lines.append(f"[查看榜单]({url})")
            content = "\n".join(lines)
        else:
            # 精选联盟字段（数值型）
    ...
    card = {
        ...
        "header": {
            "title": {
                "content": f"🛒 抖音热销榜 · {category}（蝉妈妈）" if is_chanmama
                           else f"🛒 精选联盟热销榜 · {category}",
                "tag": "plain_text"
            },
            "template": "purple",
        },
    ...
    _send_feishu_card(webhook, card, f"抖音{category}热销榜已更新")
